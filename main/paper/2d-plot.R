...
## Temporary;
pvals.master[1:3000,6] = runif(3000,0,1)
## load(file=file.path(outputdir,"myfilename.Rdata"))
## pvals.master[,6] = pvals[,6]
...
unif.p = runif(3000,0,1)

cols = brewer.pal(6,"Set1")[c(6,5,2,4,3,1)]
pch=16
for(igrain in c(6,3,5)){#1:length(lev2list)
  print(igrain)
  lev2 = lev2list[igrain]
  if(igrain != 6 ) par(new=T)
  a = qqplot(x=unif.p, y=pvals.master[,igrain],plot.it=FALSE)
  myfun = (if(igrain==6)plot else points)
  myfun(x=a$y, y=a$x, col=cols[igrain],pch=pch, axes=F, xlab="", ylab="")
}
axis(2);axis(1)
mtext("Observed",2,padj=-4)
mtext("Expected",1,padj=4)
legend("bottomright", col=cols[c(6,3,5)], pch=rep(pch,2),
       legend = sapply(c(bquote(delta == .(lev2list[6])), 

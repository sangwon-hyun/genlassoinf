...
## solves the fused 1d lasso path for response matrix
#' |y| is the response (this is signal approximation, so no x required)
#' |verbose| if you like loud code.
#' |plotind| the indices of the path knots to be plotted e.g. 2:8. Default is NA, which plots everything.
fl1d = function(y, maxsteps = length(y), verbose = F){
  m = length(y)
  uhatlist = matrix(nrow = m-1, ncol = m-1)
  Gammat = rep(NA, m)
  D = dual1d_Dmat(m)
  ...
  D1 = D #D[-B,]
  D2 = NA #D[B,]
  inds = (1:(m-1)) # unnessary in further steps.
  uhat = svdsolve(t(D1),y)$x#lm( y ~ .-1, data = as.data.frame(t(D1)))$coef
  a = svdsolve(t(D1),y)$x#solve(y)
  b = 0 #solve(y)
  hits = a/(b+sign(a))
  ll[k+1] = max(hits) # 
  B[k+1] = which.max(hits)
  s[k+1] = sign(a)[which.max(hits)]
  uhatzero = uhat
  
  
  ## add rows to \Gamma
  ## rows to add, for sign-eligibility
  tDinv = pinv(t(D1))
  Gammat = rbind(Gammat, diag(as.numeric(sign(a))) %*% tDinv) ## adds sign(a)[kk] * tDinv[kk,] for each kk
  Gammat = Gammat[-1,]
  
  ## rows to add, for selection event
  len = length(sign(a))  # should be n-k-1
  jk = which.max(hits)
  for(mm in 1:len){
    c1 = ( tDinv[jk,] ) / ( 0 + sign(a)[jk]) ## the first term in the demoninator is zero in the first run at k=0.
    c2 = ( tDinv[mm,] ) / ( 0 + sign(a)[mm])
    c_minus_c = c1 - c2
    Gammat = rbind(Gammat, c_minus_c)
  }
  inds = (1:(m-1))[-B]
  

  # getting rid of all-zero rows
  redundant = (1:nrow(Gammat))[apply(Gammat, 1, function(row){all(row==rep(0,ncol(Gammat)))})] 
  Gammat = Gammat[-redundant,]
  
  nk = nrow(Gammat)
  if(min(Gammat%*%y) < 0 ) stop("Gamma y has negative entry!")

  k=1
  while(k <= maxsteps-1){ # quick test would be to see if length(B) == k
    D1 = D[-B,]
    D2 = (if(k==1) t(D[B,]) * s else t(D[B,]) %*% s )
    
    ## obtain interior + boundary estimates for step k
    uhat = numeric(m-1)
    tD1 = if(is.null(dim(D1))) D1 else t(D1)
    if(length(B) != (m-1)) uhat[-B] = svdsolve(tD1,as.numeric(y-ll[k]*D2))$x
    uhat[B] = ll[k]*s
    uhatlist[,k] = uhat
    
    ## checking whether interior coordinates are inside ll[k] and -ll[k]
    #if(!all(abs(uhatlist[,k]) <= ll[k]) ) stop("interior coordinates are not in bounds!")
    
    if(k==(m-1)) break ## no further updates needed at end of path;
    
    ## updates for step k+1
    a    = svdsolve(tD1,as.numeric(y))$x
    b    = svdsolve(tD1,as.numeric(D2))$x
    hits = a/(b+sign(a))
  
    ll[k+1] = max(hits)
    B[k+1]  = inds[which.max(hits)]
    s[k+1]  = sign(a)[which.max(hits)]
    
    ## add rows to \Gamma
    ## rows to add, for eligibility
    tDinv = ginv(tD1)
    signs = if(length(as.numeric(sign(a)))==1) as.numeric(sign(a)) else diag(as.numeric(sign(a)))
    Gammat = rbind(Gammat, signs%*% tDinv) ## adds sign(a)[kk] * tDinv[kk,] for (kk in ind)
    ## rows to add, for selection event 
    len = length(sign(a))  # should be n-k-1
    jk = which.max(hits)  # NOT B[k+1]
    
    c1 = ( tDinv[jk,] ) / (( tDinv %*% (if(k==1) t(D2) else D2 ))[jk] + sign(a)[jk])
    for(mm in 1:len){
      c2 = ( tDinv[mm,] ) / (( tDinv %*% (if(k==1) t(D2) else D2 ))[mm] + sign(a)[mm])
      c_minus_c = c1 - c2
      Gammat = rbind(Gammat, c_minus_c)
    }
    inds = (1:(m-1))[-B]  
  ...
  colnames(uhatlist) = c(round(ll,3), rep(NA, ncol(uhatlist)-length(ll)))
  toReturn = list(u = uhatlist,
                  s = s,
                  B = B,
                  ll = ll,
                  nk = nk,
...
#' Function to plot the primal or dual variable path
#' Example usage: plot.fl1d(fl1d(..))
plot.fl1d = function(fl1d.obj, type = "dual", plotind = NA){
  if(is.na(plotind)) {
    ee1 = 1
    ee2 = nrow(fl1d.obj$u)
  }  else{
  ...
  if(type == "dual"){
    uhatlist <- fl1d.obj$u
    ll <- fl1d.obj$ll
    ylim=range(t(uhatlist)[ee1:ee2,], na.rm=T)
    xlim=c(0,max(t(uhatlist)[ee1:ee2,],na.rm=T)+1)
    matplot(y = t(uhatlist)[(ee1:ee2),], x = ll[(ee1:ee2)], type = 'o', pch = 1, main = "dual", ylab = "dual coordinates", xlab = "lambda", ylim = ylim, xlim = xlim)
    abline(0,1, lwd = 2)
    abline(0,-1, lwd = 2) 
  } else if (type == "primal"){
    print("Not coded yet, but coming soon")
  } else { 
    print ("unrecognized |type| argument: please use 'dual' or 'primal' ")
  } 
  return(0)
}

#' produces primal solution from dual solution on a point in path
getprimal = function(u, y, type = "fl1d"){
  if(type == "fl1d"){
...
##' get k'th Gammat matrix
##' obj : either output from fl1d() or from dualpathSvd2()
##' Note! This is an old version that combines the IC conditioning and
##'       selection step (naive) conditioning. I later separated this into two steps.
#getGammat = function(obj, y, k, usage = c("fl1d", "dualpathSvd"), stoprule = c("none","bic","aic","bic2","ebic","ebic.strong","sbic"), sigma = -100, consec=2, maxsteps=length(y),tf.order=0){
...
#  if(length(obj$action) < k ) stop("\n You must ask for polyhedron from less than ", length(obj$action), " steps! \n")
#  if(stoprule %in% c("bic","aic") & sigma==-100) stop("Provide noise level to getGammat().")
#  if(length(obj) < 11 && "completepath" %in% objects(obj)) stop("|obj| and |usage| are mismatched")
##  if(k<=0 || k >= n) stop("k must be within 1,2,.. (length(y)-1)")

#  # get appropriate sized Gammat from step k
#  if(usage == "fl1d"){
#    myGammat = obj$Gammat[1:obj$nk[k],]
#  } else {
#    myGammat = obj$Gamma[1:obj$nk[k],]
#  }

#  # function to get the sequence of information criterion values
#  getic = function(stoprule,y,obj,sigma,maxsteps,n,tf.order){
#    if(stoprule=='bic'){
#      ic = getbic(y,obj,sigma,maxsteps,k=tf.order)
#    } else if (stoprule == 'bic2'){
#      ic = getbic(y,obj,sigma,maxsteps,bic.fac=2)
#    } else if (stoprule == 'ebic'){
#      ic = getebic(y,obj,sigma,maxsteps,ebic.fac=.5)
#    } else if (stoprule == 'ebic.strong'){
#      ic = getebic(y,obj,sigma,maxsteps,ebic.fac=1E-5)
#    } else if (stoprule == 'sbic'){
#      ic = getbic(y,obj,sigma,maxsteps,strength=1.01)
#    } else if (stoprule == 'aic'){
#      ic = getaic(y,obj,sigma,maxsteps)
#    }
#    return(ic)
#  }

#  
#  # function to get the constant leftover in BIC comparisons (like in RSS1-RSS2>const)
#  getconst = function(stoprule,step,sigma,n){
#    if(stoprule=='bic'){
#      const = (sigma^2)*(log(n))
#    } else if (stoprule == 'bic2'){
#      const = (sigma^2)*(log(n))*2
#    } else if (stoprule == 'ebic'){
#      ebic.fac = .5
#      const = (sigma^2)*(log(n)) + 2*(1-ebic.fac)*(log(choose(n,step)) - log(choose(n,step-1)))
#    } else if (stoprule == 'ebic.strong'){
#      ebic.fac = 1E-5
#      const = (sigma^2)*(log(n)) + 2*(1-ebic.fac)*(log(choose(n,step)) - log(choose(n,step-1)))
#    } else if (stoprule == 'sbic'){
#      const = (sigma^2)*(log(n))^1.01
#    } else if (stoprule == 'aic'){
#      const = 2*(sigma^2)
#    }
#    return(const)
#  }
#    
#  upol = rep(0, nrow(myGammat))
#  
#  if(stoprule %in% c('bic','aic', 'bic2','ebic','sbic','aic')){
#    
#    ic = getic(stoprule,y,obj,sigma,maxsteps,n,tf.order)

#    # s* : change in model size
#    actiondirs = c(NA, sign(obj$action)[1:(maxsteps)])
...
#      residual = makeresid(prevhits = small.model, 
#                           thishit  = this.hit, n=n, k = tf.order) 
#                           
#      const = getconst(stoprule,jj,sigma,n) 
#      
#      upol = c(upol, -signs[jj+1]*sqrt(const))
#      rows.to.add[jj,] = (-signs[jj+1]) * sign(t(residual)%*%y) * residual/sqrt(sum(residual^2))
#    }

#    # TODO: in the graph case, only add to Gamma and |u| when
#    #        new partition is created

#    myGammat = rbind(myGammat, rows.to.add)
#  }
#  
#  if(stoprule == 'none'){
#    return(myGammat)
#  } else if (stoprule %in% c('bic','aic','bic2','ebic','ebic.strong')){
#    if(nrow(myGammat)!=length(upol)) stop('dimensions dont match, check myGammat and upol')
#    return(list(Gammat = myGammat, u = upol))
#  } else {
#    stop('stoprule not coded yet!')
#  }
#}
...
#' get k'th Gammat matrix
#' obj : either output from fl1d() or from dualpathSvd2()
#' Note! if you're being naive, then do whatever you want with condition.step.
#        if you're using stop times, condition.step should be the (stoptime + 2)
...
## Assumes usage of dualPathSvd2
## Produces the rows to add to Gammat matrix.
#getGammat.stoprule.tf = function(obj, y, condition.step, stoprule, sigma, consec, maxsteps, tf.order=0, bic.fac=1, ebic.fac= 0.5){

#  n = length(y)
#  if(stoprule %in% c("bic","aic") & sigma==-100) stop("Provide noise level to getGammat().")

#  # Function to get the entire sequence of information criterion values
#  getic = function(stoprule,y,obj,sigma,maxsteps,tf.order,bic.fac, ebic.fac){
#    if(stoprule=='bic'){
#      ic = getbic(y,obj,sigma,maxsteps,k=tf.order,bic.fac)
#    } else if (stoprule == 'bic2'){
#      warning("Only coded for 1d fused lasso!")
#      ic = getbic2(y,obj,sigma,maxsteps,fac=2)
#    } else if (stoprule == 'ebic'){
#      ic = getebic(y,obj,sigma,maxsteps,ebic.fac=ebic.fac, k=tf.order)
#    } else if (stoprule == 'sbic'){
#      warning("Only coded for 1d fused lasso!")
#      ic = getbic(y,obj,sigma,maxsteps,strength=1.01)
#    } else if (stoprule == 'aic'){
...
#  # Function to get the entire set of constants leftover 
#  # in BIC comparisons (like in RSS1-RSS2>const)
#  getconst = function(stoprule,step,sigma,n,big.df=NA, small.df=NA,bic.fac=1,ebic.fac=.5){
#    if(stoprule=='bic'){
#      const = (sigma^2)*(log(n))*bic.fac
#    } else if (stoprule == 'bic2'){
#      const = (sigma^2)*(log(n))*2
#    } else if (stoprule == 'ebic'){
#      #const = (sigma^2)*(log(n)) + 2*(1-.5)*(log(choose(n,step)) - log(choose(n,step-1)))
#      const = (sigma^2)*(log(n)) + 
#               (sigma^2) * 2*(1-ebic.fac) * (log(choose(n,big.df)) - log(choose(n,small.df)))
#    } else if (stoprule == 'sbic'){
#      const = (sigma^2)*(log(n))^1.01
#    } else if (stoprule == 'aic'){
#      const = 2*(sigma^2)
...
#  # Helper function to get trend filtering degrees of freedom of |model|, where..
#  # |model| : index set of the dual variables chosen so far
#  #           (_not_ including the always-included bases (tf.order+1))
#  get.tf.df = function(model, tf.order){
#    model = model[!is.na(model)]
#    return(length(model) + (tf.order + 1))
#  }

#  # Helper function to get the graph fused lasso degrees of freedom of |model|, where..
#  # |model| : index set of the dual variables chosen so far
#  #           (_not_ including the always-included bases (tf.order+1))
#  get.graph.df = function(model, tf.order){
#    model = model[!is.na(model)]
#    return(length(model))
#  }

#  ic = getic(stoprule,y,obj,sigma,maxsteps,tf.order, bic.fac=bic.fac, ebic.fac=ebic.fac)
...
#    small.model = (if(actiondirs[jj+1]==+1){prev.state} else {this.state})    
#    
#    big.df   = get.tf.df(big.model, tf.order)
#    small.df = get.tf.df(small.model, tf.order)
#    
#    this.hit = big.model[!(big.model %in% small.model)] 
#    residual = makeresid.tf(prevhits = small.model, 
#                            thishit  = this.hit, n=n, k = tf.order) 
#                         
#    const = getconst(stoprule,jj,sigma,n,big.df,small.df,bic.fac,ebic.fac)
#   
##    ## Checking the constants, for EBIC (modify for use in other ICs)
##    ebic = getic('ebic',y,obj,sigma,maxsteps,tf.order)

##    small.model = small.model[!is.na(small.model)]
##    (BIC.small = ebic[1])
##    H0.small = H[,c(1,2,(small.model+1))]
##    (RSS.small = sum((resid(lm(y0~H0.small-1)))^2))
##    ii=1
##    df = length(states[[ii]][!is.na(states[[ii]])]) + tf.order + 1
##    sizepenalty = 2*(1-fac)*log(choose(n,df))*sigma^2 
##    (const.small = df*log(length(y0))*sigma^2 + sizepenalty)
##    
##    (BIC.big = ebic[2])
##    H0.big = H[,c(1,2,(big.model+1))]
##    (RSS.big = sum((resid(lm(y0~H0.big-1)))^2) )
##    ii=2
##    df = length(states[[ii]][!is.na(states[[ii]])]) + tf.order + 1
##    sizepenalty = 2*(1-fac)*log(choose(n,df))*sigma^2 
##    (const.big = df*log(length(y0))*sigma^2 + sizepenalty)
##    
##    all.equal(const , const.big-const.small)    
##    ## End of check
#    
#    
...
# Produces the rows to add to Gammat matrix for IC-based stopping rule
# in generalized lasso signal approximation problem.
# This handles applications including (1d Fused Lasso TF, Graph fused lasso)
getGammat.stoprule = function(obj, y, condition.step, stoprule, sigma, consec, maxsteps, ebic.fac= 0.5, D, verbose=F){

  if(!(stoprule %in% c('bic','ebic','aic'))){    stop('stoprule not coded yet!') }
  
  n = length(y)
  if(verbose) print("Beginning of BIC calculation")
  mm = get.modelinfo(obj,y,sigma,maxsteps=maxsteps,D=D,stoprule=stoprule,ebic.fac=ebic.fac,verbose=verbose)
  if(verbose) print("End of BIC calculation")
...
## Assumes usage of dualPathSvd2
## Produces the rows to add to Gammat matrix for the IC-based stopping rule.
#getGammat.stoprule.graph = function(obj, y, condition.step, stoprule, sigma, consec, maxsteps, bic.fac=1, graphobj){
#  if(stoprule!='bic'){
#    stop("Graph stopping rule only coded for BIC.")
#  }
...
#  # Check if conditioning steps are the same (i.e. stoptime + consec)
#  stopifnot(condition.step == stoptime+consec)
#  if(condition.step != stoptime+consec){ print("BIC and IC have small numerical difference, move on.."); next}

#  # safely making enough empty rows to add
#  more.rows.to.add = rows.to.add = matrix(NA, nrow = (stoptime+consec),
#                                               ncol = ncol(obj$Gammat))
#  more.upol = upol = rep(NaN,stoptime+consec-1)
...
#  
#    # Get the residuals
#    residual = graphobj$residuals[[jj+1]]
#    
#    # Get the constant
#    const = (sigma^2)*(log(n))*bic.fac*1
#    
#    # Add it
#    upol[jj] = (-signs[jj+1]) * sqrt(const)
#    rows.to.add[jj,] = (-signs[jj+1]) * sign(t(residual)%*%y) * residual/sqrt(sum(residual^2))
#    
#    # Also add other direction if the model size change and BIC comparison are both up or down in direction
#    if(actiondirs[jj+1] == seqdirs[jj+1] & !any(is.na(residual))){
#      more.upol[jj] = (-signs[jj+1]) * sqrt(const)
#      more.rows.to.add[jj,] = (signs[jj+1]) * sign(t(residual)%*%y) * residual/sqrt(sum(residual^2))
#    }
...
# Produces the rows to add to Gammat matrix, in the regression case
# requires X.orig, ginvX.orig and D.orig which are from the pre-elastic-net regression problem
getGammat.stoprule.regression = function(obj, y, condition.step, stoprule, sigma, consec, maxsteps,  X.orig, ginvX.orig=NULL, D.orig,y0.orig){
  if(stoprule!='bic'){
...
# Function that takes the same arguments as the naive one
# and some extra arguments to extract the stoptime.
# and returns the entire Gamma matrix and u vector with stopping rules incorporated
getGammat.with.stoprule = function(obj, y, condition.step,# usage = c("fl1d", "dualpathSvd"),
                                   stoprule, sigma, maxsteps, consec, 
                                   ebic.fac= 0.5, type = c("tf","graph", "regression"), 
                                   X=NULL, D=NULL, X.orig=NULL, ginvX = NULL, D.orig=NULL, y0.orig = NULL,ginvX.orig = NULL,verbose=F){
  type = match.arg(type)
  if(type=="regression" & (is.null(y0.orig)|is.null(X.orig)|is.null(D.orig))) stop("You need to supply y, design matrix and D matrix from pre-elastic-net regression problem!")
  ...
  if(type %in% c("tf", "graph")){
    G.new.obj = getGammat.stoprule(obj=obj, y=y, condition.step=condition.step, 
                                     stoprule=stoprule, sigma=sigma, consec=consec, 
                                     maxsteps=maxsteps, ebic.fac= ebic.fac, D=D, verbose=verbose)
#  } else if (type == "graph"){
#    G.new.obj = getGammat.stoprule.graph(obj=f0,y=y0,
#                                       condition.step = condition.step,
#                                       stoprule = "bic", sigma=sigma, bic.fac=bic.fac,
#                                       consec=consec, maxsteps=maxsteps, graphobj=graphobj)
  } else if (type == "regression"){
    G.new.obj = getGammat.stoprule.regression(obj=obj,y=y, y0.orig = y0.orig,
                                       condition.step = condition.step,
                                       stoprule = "bic", sigma=sigma,
                                       consec=consec, maxsteps=maxsteps, X.orig = X.orig, D.orig = D.orig, ginvX.orig=ginvX.orig)#)
  } else {
  ...
  stopifnot(length(uvec)==nrow(G.combined))
...
# Written by Max G'sell
# Simple approximation for integrating normal tails, 
# and for calculating pvalues in truncated normals.
# Right now, the n=1000 is just a reasonable seeming choice,
# I have not finished proving error bounds.

#Test functions, will break down with z values around 10
exact_int = function(a,b){
  pnorm(b)-pnorm(a)
}
exact_frac = function(a,b,x){
  (pnorm(b)-pnorm(x))/(pnorm(b)-pnorm(a))
}
...
approx_int = function(a,b,n=1000,offset=0){
  if(b > 15) b = a+10 # no clear reason for 15 or 10; trying to catch b=Inf
...
approx_frac = function(a,b,x,n=1000){
...
### Returns sequence of BIC of fused lasso regression model (general regressor matrix X)
### The variable names 00.orig are to emphasize that while the f0 may be from solving a
### ridge-penalty-added problem, what should be provided are the original, non-augmented versions
###
...
## Calculates various model-related quantities for _any_ D for the signal approximator case
## (IC, penalty, RSS, residual)
## df.fun is a function that returns the degrees of freedom of fit of yhat = Proj_{null(D_B)} y
...
#      y0.fitted2 = null.proj %*% y0
    ...
    mypen = (if(stoprule=='bic'){   (sigma^2) * mydf * log(n) 
            } else if (stoprule=='ebic'){  (sigma^2) * mydf * log(n) + 
                                       2*(1-ebic.fac) * log(choose(n,mydf)) * sigma^2
            } else if (stoprule=='aic'){ (sigma^2) * mydf * 2 
            } else { stop(paste(type, "not coded yet!"))})
...
## Calculates BIC for _any_ D for the signal approximator case
## df.fun is a function that returns the degrees of freedom of fit of yhat = Proj_{null(D_B)} y
...
#getbic.general = get.ic(y0, f0, sigma, maxsteps, ebic.fac, D, type='bic')
#getic.tf = function(y0, f0, sigma, maxsteps, k=0){
#  return(getic(y0,f0,sigma,maxsteps,bic.fac,k=0))
#}



### Returns sequence of BIC of a fused lasso model (or more generally, k'th order trend filtering models)
### BIC = RSS + sigma d log N 
#getbic.tf = function(y0, f0, sigma, maxsteps, bic.fac=1, strength=1, k=0, more=F){

#  # Setup empty IC vector and regressor matrix H, for k'th order trend filtering problems
#  bic = pen = RSS = numeric(maxsteps+1)
#  n = length(y0)  
#  H = getH.trendfilter(n,k)
...
#  for(ii in 1:(maxsteps+1)){
#    thishits = states[[ii]]
#    basis.cols = c((1:(k+1)), thishits+1)
#    basis.cols = basis.cols[!is.na(basis.cols)]
#    H0 = H[,basis.cols]
#    mydf = (if(ii==1) k+1 else length(states[[ii]]) + k + 1)
#    myRSS = sum((y0 - fitted(lm(y0~H0-1)))^2)
#    mypen = (sigma^2) * bic.fac * mydf * (log(length(y0)))^strength
#    
#    RSS[ii] <- myRSS
#    pen[ii] <- mypen
#    bic[ii] <- mydf + myRSS
...
#  }
#  if(more){
#    return(list(bic=bic,pen=pen,bic=bic))
#  }
#  return(bic)
#}
...
### Returns sequence of BIC of a trend filtering model
### BIC = RSS + sigma d log N + sizepenalty
#getebic.tf = function(y0, f0, sigma, maxsteps, ebic.fac = .5, k=0){
...
#  # Collect eBIC at each step 0 ~ (maxsteps-1)
#  ebic=c()
#  for(ii in 1:(maxsteps+1)){
#    thishits = states[[ii]]
#    basis.cols = c((1:(k+1)), thishits+1)
#    basis.cols = basis.cols[!is.na(basis.cols)]
#    H0 = H[,basis.cols]
#    df = length(states[[ii]][!is.na(states[[ii]])]) + k + 1
#    sizepenalty = 2*(1-ebic.fac)*log(choose(n,df))*sigma^2 
#    ebic[ii] <- sum((y0 - fitted(lm(y0~H0-1)))^2) + 
#                df*log(length(y0))*sigma^2 + 
#                sizepenalty
#  }
#  return(ebic)
#}
...
### Returns sequence of AIC of a 0th order trend filtering
#getaic = function(y0, f0, sigma, maxsteps,k=0){
# 
#  # Setup empty IC vector and regressor matrix H, for k'th order trend filtering problems
#  aic = numeric(maxsteps+1)
#  n = length(y0)  
#  H = getH.trendfilter(n,k)
...
#    basis.cols = basis.cols[!is.na(basis.cols)]
#    H0 = H[,basis.cols]
#    df = length(states[[ii]]) + k + 1
#    aic[ii] <- sum((y0 - fitted(lm(y0~H0-1)))^2) + 
#                (sigma^2) * df * 2
#  }
#  return(aic)
#}
...
# Temporary test function, to see endpoints Vlo and Vup
getVs = function(y, G, dik, sigma, u = rep(0, nrow(G))){
...
# takes in a D matrix used in the generalized lasso
# and creates an adjacency matrix (a matrix with zero entries 
#                                  in non-adjacent edges and 1 in adjacent edges)
...
#  Wrapper for getmodelinfo.graph() to get bic scores only
#  Returns BIC for steps 0~maxsteps
        ...
        for(jj in 1:prev.nclust){
          for(kk in 1:curr.nclust){
            if( setequal(prev.groups[[jj]], curr.groups[[kk]])){
              newbasis[prev.groups[[jj]]] = 0 # if _any_ of the previous groups match with current groups exactly, assign zero

...
klater = 2
G = getGammat.naive(f0,y0,condition.step=klater)#k=klater,maxsteps=maxsteps,"dualpathSvd")
...
for(ii in 1:klater){
  d.spike = getdvec(f0,y0,k=ii,klater=klater,type="spike",usage="dualpathSvd",matchstep=F)
  d.segment = getdvec(f0,y0,k=ii,klater=klater,type="segment",usage="dualpathSvd",matchstep=F)
